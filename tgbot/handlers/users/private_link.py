...
async def sub_link_private(call: CallbackQuery):
    ...
    if db.get_sub_status(call.from_user.id):
        await call.bot.send_message(call.from_user.id, "<b>Ваши приватные ссылки для доступа</b> 👇\n\n"
...
async def sub_link_open(call: CallbackQuery):
    await call.bot.delete_message(call.from_user.id, call.message.message_id)
    if db.get_sub_status(call.from_user.id):
        await call.bot.send_message(call.from_user.id, "✅ Вход открыт, вступайте (нажмите на кнопку ниже):",
